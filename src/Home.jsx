...
function Home() {
  const vegItems = useSelector(state => state.product.veg).slice(0, 6);
  const nonVegItems = useSelector(state => state.product.nonVeg).slice(0, 6);
  const dispatch = useDispatch();
  ...
  const renderCard = (item, index) => (
    <div className="col-md-3 mb-4" key={index}> {/* Change col-md-4 to col-md-3 */}
      <div className="card h-100">
        <img
          src={item.image}
          alt="Not found"
